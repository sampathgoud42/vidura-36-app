"""
bot_kalshi_silver15.py — Silver 15-minute signal follower for Kalshi KXSILVER15M markets.

Kalshi client + API usage lifted from v5_bot_btc_15_2.py (RSA-PSS signed
V2 endpoints).  Strategy identical to btc15 v5, adapted for silver:

  * whenever a NEW SILVER-15 market opens, derive its quarter mark T
  * refresh the signal CSV via the silver quarter bat at :02/:17/:32/:47 cadence
...
SIGNAL_CSV  = Path(os.getenv("SILVER15_SIGNAL_CSV",
                             str(_INDICATORS / "silver15_signal_report.csv")))
QUARTER_BAT = Path(os.getenv("SILVER15_QUARTER_BAT",
...
TRADES_CSV = Path(os.getenv("BOTSILVER_CSV_PATH", "bot_silver15_trades.csv"))
STATE_FILE = Path(os.getenv("BOTSILVER_STATE_PATH", "bot_silver15_state.json"))
LOG_FILE   = Path(os.getenv("BOTSILVER_LOG_PATH", "silver-15.log"))
...
class KalshiClient:
    def __init__(self) -> None:
        ...
        if not pem.is_absolute():
            pem = Path.cwd() / pem
        if not pem.exists():
            sys.exit(f"bot_kalshi_silver15: private key not found: {pem}")
        raw = pem.read_bytes()
        ...
        try:
            if os.name == "nt" and QUARTER_BAT.is_file():
                ...
                proc = await asyncio.create_subprocess_exec(
                    ...
                    "--hours", str(REFRESH_HOURS), "--symbols", "silver15",
                    cwd=str(QUARTER_BAT.parent),
                    stdout=asyncio.subprocess.DEVNULL, stderr=asyncio.subprocess.DEVNULL)
                await asyncio.wait_for(proc.wait(), timeout=240)
...
_TRADE_COLS = ["timestamp_ct", "ticker", "mark", "action", "direction", "side",
               ...
               "best_move", "silver_current15", "prev_matched", "prev_momentum"]
...
def log_trade(ticker: str, mark: datetime, row: dict, side: str, resp: dict | None,
    ...
    with open(TRADES_CSV, "a", newline="", encoding="utf-8") as f:
        ...
        w.writerow([f"{_now_ct():%m/%d/%Y %H:%M:%S}", ticker, f"{mark:%m/%d/%Y %H:%M}",
                    ...
                    row.get("silver_current15", ""), prev_m, prev_mom])
...
async def handle_market(c: KalshiClient, m: dict, st: dict) -> None:
    ...
    try:
        sc = await asyncio.to_thread(score_commodity, "silver15", True)
...
async def main() -> None:
    log(f"bot_kalshi_silver15 starting  DRY_RUN={DRY_RUN}  contracts={CONTRACTS}  "
