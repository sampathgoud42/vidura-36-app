"""
bot_kalshi_oil15.py — WTI Oil 15-minute signal follower for Kalshi KXOIL15M markets.

Kalshi client + API usage lifted from v5_bot_btc_15_2.py (RSA-PSS signed
V2 endpoints).  Strategy identical to btc15 v5, adapted for WTI oil:

  * whenever a NEW OIL-15 market opens, derive its quarter mark T
  * refresh the signal CSV via the commodity quarter bat at :02/:17/:32/:47 cadence
...
Env (independent flags so this bot cannot go live by accident):
  BOTOIL_DRY_RUN    TRUE/FALSE   default TRUE
  BOTOIL_CONTRACTS  int          default 1
"""
...
DRY_RUN     = os.getenv("BOTOIL_DRY_RUN", "TRUE").upper() == "TRUE"
CONTRACTS   = int(os.getenv("BOTOIL_CONTRACTS", "1"))
MIN_CENTS   = int(os.getenv("BOTOIL_MIN_CENTS", "35"))
MAX_CENTS   = int(os.getenv("BOTOIL_MAX_CENTS", "49"))
BUY_AT_CENTS = MAX_CENTS
WIDE_TRIGGER_CENTS = int(os.getenv("BOTOIL_WIDE_TRIGGER_CENTS", "60"))
WIDE_MAX_CENTS     = int(os.getenv("BOTOIL_WIDE_MAX_CENTS", "55"))
TP_CENTS    = int(os.getenv("BOTOIL_TP_CENTS", "90"))
TP_AT_MIN   = int(os.getenv("BOTOIL_TP_AT_MIN", "10"))
BAND_POLL_SEC    = float(os.getenv("BOTOIL_BAND_POLL_SEC", "1"))
CLOSE_BUFFER_SEC = int(os.getenv("BOTOIL_CLOSE_BUFFER_SEC", "180"))
...
SIGNAL_CSV  = Path(os.getenv("OIL15_SIGNAL_CSV",
                             str(_INDICATORS / "oil15_signal_report.csv")))
QUARTER_BAT = Path(os.getenv("OIL15_QUARTER_BAT",
                             str(_INDICATORS / "commodity15_quarter.bat")))
REFRESH_HOURS    = "2"
SIGNAL_CHECK_SEC = int(os.getenv("BOTOIL_SIGNAL_CHECK_SEC", "150"))
SIGNAL_POLL_SEC  = float(os.getenv("BOTOIL_SIGNAL_POLL_SEC", "2"))
REFRESH_MINUTES  = (2, 17, 32, 47)

TRADES_CSV = Path(os.getenv("BOTOIL_CSV_PATH", "bot_oil15_trades.csv"))
STATE_FILE = Path(os.getenv("BOTOIL_STATE_PATH", "bot_oil15_state.json"))
LOG_FILE   = Path(os.getenv("BOTOIL_LOG_PATH", "oil-15.log"))
...
class KalshiClient:
    def __init__(self) -> None:
        ...
        if not pem.is_absolute():
            pem = Path.cwd() / pem
        if not pem.exists():
            sys.exit(f"bot_kalshi_oil15: private key not found: {pem}")
        raw = pem.read_bytes()
        ...
        try:
            if os.name == "nt" and QUARTER_BAT.is_file():
                ...
                proc = await asyncio.create_subprocess_exec(
                    ...
                    "--hours", str(REFRESH_HOURS), "--symbols", "oil15",
                    cwd=str(QUARTER_BAT.parent),
                    stdout=asyncio.subprocess.DEVNULL, stderr=asyncio.subprocess.DEVNULL)
                await asyncio.wait_for(proc.wait(), timeout=240)
...
_TRADE_COLS = ["timestamp_ct", "ticker", "mark", "action", "direction", "side",
               ...
               "best_move", "oil_current15", "prev_matched", "prev_momentum"]
...
def log_trade(ticker: str, mark: datetime, row: dict, side: str, resp: dict | None,
    ...
    with open(TRADES_CSV, "a", newline="", encoding="utf-8") as f:
        ...
        w.writerow([f"{_now_ct():%m/%d/%Y %H:%M:%S}", ticker, f"{mark:%m/%d/%Y %H:%M}",
                    ...
                    row.get("oil_current15", ""), prev_m, prev_mom])
...
async def tp_seller(c: KalshiClient, ticker: str, side: str, mark: datetime) -> None:
    ...
    #
    # The side comes from the position rather than from the signal: the guard
...
async def handle_market(c: KalshiClient, m: dict, st: dict) -> None:
    ...
    try:
        sc = await asyncio.to_thread(score_commodity, "oil15", True)
...
async def main() -> None:
    log(f"bot_kalshi_oil15 starting  DRY_RUN={DRY_RUN}  contracts={CONTRACTS}  "
