# Gold-15 — default engine (v3, the V7 DMI-stack engine)

Default version per `app/domains/botstation/builtin.py`: **v3**
(`v3_bot_kalshi_gold15.py`). That file is ~20 lines: the engine itself is
...
Read gold's own row on the desk's **COMMODITIES** board — the same module the
