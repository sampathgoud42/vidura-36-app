"""
v2_bot_kalshi_gold15.py — Gold 15-minute DMI signal follower for Kalshi KXGOLD15M markets.

Kalshi client + market/order/TP plumbing lifted unchanged from
bot_kalshi_gold15.py (v1). The only thing v2 replaces is the signal: instead
...
TRADES_CSV = Path(os.getenv("BOTGOLD_CSV_PATH", "v2_bot_gold15_trades.csv"))
STATE_FILE = Path(os.getenv("BOTGOLD_STATE_PATH", "v2_bot_gold15_state.json"))
LOG_FILE   = Path(os.getenv("BOTGOLD_LOG_PATH", "v2_gold-15.log"))
...
class KalshiClient:
    def __init__(self) -> None:
        ...
        if not pem.is_absolute():
            pem = Path.cwd() / pem
        if not pem.exists():
            sys.exit(f"v2_bot_kalshi_gold15: private key not found: {pem}")
        raw = pem.read_bytes()
...
async def handle_market(c: KalshiClient, m: dict, st: dict) -> None:
    ...
    while _now_ct() < band_deadline:
        try:
            sc = await asyncio.to_thread(score_commodity_dmi, "gold15", True)
        except Exception as e:
...
async def main() -> None:
    log(f"v2_bot_kalshi_gold15 starting  DRY_RUN={DRY_RUN}  contracts={CONTRACTS}  "
