...
@router.get("/commodities", operation_id="getTradierCommodities")
def commodities(
    user_id: str = Query(...),
    live: bool = Query(default=False),
    refresh: bool = Query(default=False, description="force a fresh scan"),
    db: Session = Depends(get_db),
) -> dict:
    """Gold / Silver / WTI 1m + 2m DMI readings for binary trading signals."""
    from app.services import hot_scan as hot_svc

    user = _user_or_404(db, user_id)
    try:
        return hot_svc.commodities_scan(user, live=live, force=refresh)
    except Exception as exc:  # noqa: BLE001
        raise _translated(exc) from exc
...
class CarryOverRequest(BaseModel):
    user_id: str
    carry_over: bool = Field(default=True,
                             description="true enables carry-over (no SL, no EOD close); "
...
def set_carry_over(position_id: int, payload: CarryOverRequest,
                   db: Session = Depends(get_db)) -> dict:
    """Toggle carry-over on a live position.

    Carry-over removes the stop-loss watch and skips the EOD auto-close,
    letting the position hold overnight. The take-profit order stays on
    the venue.
    """
    require_local_runtime("Toggling carry-over")
    user = _user_or_404(db, payload.user_id)
    from app.models.tradier import TradierPosition
    ...
    if pos.status not in ("open", "pending"):
        raise HTTPException(status_code=400,
                            detail=f"cannot carry over a {pos.status} position")
    pos.carry_over = payload.carry_over
...
class AutoTradeStart(BaseModel):
    ...
    discount_pct: float | None = Field(
        ...
        description="limit-order discount off the mid/ask; 0 for market-price "
                    "limit (smart_limit)")
    top_n: int | None = Field(
        default=None, ge=1, le=20,
        description="super_hot_tickers: how many top superhot names to trade")
    zero_dte: bool = Field(default=True,
                           description="true allows same-day expirations; false "
                                       "skips today and picks the next expiry")
    live: bool = Field(default=False,
