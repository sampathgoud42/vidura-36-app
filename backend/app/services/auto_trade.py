...
STRATEGIES = ("10min_intraday_move", "hot_tickers")
...
def _hot_place(w: dict, ticker: str, side: str) -> None:
    """Place a 10% discount limit order for a HOT-scan ticker."""
    from app.core.database import SessionLocal
    from app.models import User
    from app.services import tradier_bot

    p = w["params"]
    expiration = f"{_now_cst():%Y-%m-%d}"
    db = SessionLocal()
    try:
        user = db.get(User, w["user_id"])
        if user is None:
        ...
        pos = tradier_bot.open_position(
            ...
            strategy=p["strategy"],
            live=p["live"],
            discount_pct=p.get("discount_pct", 10),
        )
        w["attempts"].append({
            "at": f"{_now_cst():%m-%d %H:%M:%S}", "ticker": ticker,
            "signal": f"hot-{side}", "side": side, "status": "placed",
            "position_id": pos.id, "occ_symbol": pos.occ_symbol,
            "contracts": pos.contracts,
        })
        _log_event(w, f"PLACED {ticker} {side.upper()} x{pos.contracts} "
                      f"({pos.occ_symbol}) — HOT 5+15min, −{p.get('discount_pct', 10)}% limit")
    except tradier_bot.TradierBotError as exc:
        skipped = "min_contracts" in str(exc)
        w["attempts"].append({
            "at": f"{_now_cst():%m-%d %H:%M:%S}", "ticker": ticker,
            "signal": f"hot-{side}", "side": side,
            "status": "skipped" if skipped else "failed",
            "error": str(exc)[:300],
        })
        ...
        w["attempts"].append({
            "at": f"{_now_cst():%m-%d %H:%M:%S}", "ticker": ticker,
            "signal": f"hot-{side}", "side": side, "status": "failed",
            "error": str(exc)[:300],
        })
...
def start(user_id: str, *, buy_pct: float | None = None,
    ...
    if params["strategy"] != "hot_tickers" and any(
