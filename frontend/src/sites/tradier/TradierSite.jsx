...
export function HotScan({ user, live, onPick, onError, onBuy, buying, blocked }) {
  ...
  usePolling(load, snap?.refreshing ? 3_000 : 60_000, { enabled: !!user, blocked });
  ...
  return (
    <div className="tr-panel tr-hot mb-4">
      {/* Two lines in a narrow rail: what the list IS, then what drives it.
          The clock and the refresh sit with the granularity chips rather than
          on the title line — they are all controls of the same sweep, and
          three rows of header over a list this compact was mostly chrome. */}
      <div className="flex items-center gap-2 mb-1">
        <span className="tr-eyebrow" style={{ display: 'inline' }}>🔥 hot</span>
        <span className="tr-note"
      ...
      <div className="tr-hotlist tr-mono">
        {rows.map((r) => (
          <div key={r.symbol} className={`tr-hotrow ${r.side}`}>
            ...
            <button type="button" className="tr-tkr" onClick={() => onPick?.(r.symbol)}
              title={`${r.symbol} — live price, pivot points, TradingView`}>{r.symbol}</button>
            ...
            {/* Hidden until the row is hovered or focused, so the list reads
                as data and the order is one deliberate reach away. */}
            <button type="button" className={`tr-hotbuy ${r.side}`}
              disabled={buying === r.symbol}
              onClick={() => onBuy?.(r)}
              title={`buy a ${r.side.toUpperCase()} on ${r.symbol} with the desk's delta band, Buy% / TP% / SL%`}>
              {/* the colour is the side — green buys the call, red the put —
      ...
      <div className="tr-modal tr-panel" onClick={(e) => e.stopPropagation()}>
        ...
        <div className="tr-modal-grid">
          ...
          <div><span className="tr-label">Tickers</span>
            <input className="tr-input" value={isHot ? '(auto from HOT scan)' : f.tickers}
              onChange={set('tickers')} placeholder="SPY,QQQ,SPX"
              disabled={isHot} style={isHot ? { opacity: 0.45 } : undefined}
              title={isHot ? 'HOT tickers strategy auto-picks from 5min+15min scan intersection' : undefined} /></div>
          ...
          <div><span className="tr-label">Delta range</span>
            <input className="tr-input" value={isHot ? '0.30-0.50' : f.delta}
              onChange={set('delta')} placeholder="0.12-0.30"
              disabled={isHot} style={isHot ? { opacity: 0.45 } : undefined}
              title={isHot ? 'HOT tickers uses fixed delta 0.30-0.50' : 'delta band for contract selection, e.g. 0.12-0.30'} /></div>
          <div><span className="tr-label">Buy % of balance</span>
        ...
        <p className="tr-note mt-3">
          {isHot ? (
            <>
              ...
              DMI/ADX trend on two time-frames). Each qualifying ticker gets a LIMIT buy
              at 10% discount (auto-cancel after 15 min if unfilled), delta 0.30–0.50,
              using the side (CALL/PUT) from the scan. Tickers are auto-discovered from
              the HOT scan — no manual input needed. Each ticker is traded at most once
              per day.
            </>
          ) : (
            <>
        ...
        <div className="mt-4 flex flex-wrap items-center gap-3">
          <button type="button" className="tr-btn sm auto" disabled={busy}
            onClick={() => onArm(f)}>{busy ? '…' : '🤖 Arm auto-trade'}</button>
  ...
  return (
    <div className="tr-panel tr-flow mb-4">
      <div className="flex flex-wrap items-center gap-2 mb-2">
        <span className="tr-eyebrow" style={{ display: 'inline' }}>options flow</span>
          ...
          <div key={r.occ_symbol} className="tr-flowrow">
            <div className="l1">
              <button type="button" className="tr-flowsym" onClick={() => onPick?.(r.symbol)}
                title={`pivot levels for ${r.symbol}`}>{r.symbol}</button>
...
export default function TradierSite() {
  ...
  usePolling(refresh, 30_000, { enabled: !!user && !marketOffline, blocked: deskBlocked });
  ...
  usePolling(pollPositions, 6_000, {
    enabled: !!user && !marketOffline, blocked: isBlocked('positions'),
  });
  ...
  const buyHot = (r) => openTicket({
    symbol: r.symbol,
    side: r.side === 'put' ? 'put' : 'call',
    why: `ADX ${Math.round(r.adx)} · +DI ${r.plus_di} / -DI ${r.minus_di} · `
      + `${r.di_ratio == null ? '∞' : `${r.di_ratio}×`} the other side`,
  });
