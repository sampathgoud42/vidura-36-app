"""Phase 8, step 3: import into a NAMED database.

Same code path the dry run proved, pointed at a real file instead of a
temp one. The LIVE var/app.db is never opened for writing -- it is read
for its user rows and nothing else -- so this is additive and the old
desk keeps running against its own database throughout.

ORIGINAL DRY-RUN DOCSTRING FOLLOWS.
...
def main() -> int:
    ...
    target = os.environ.get("TBOT_IMPORT_TARGET", "")
    if target:
        scratch = Path(target); scratch_dir = scratch.parent
